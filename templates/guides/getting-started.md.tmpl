...
`terraform init` downloads the provider. `terraform plan` shows you what Terraform is about to do; read it before approving. `terraform apply` creates the project, calls `instances/create`, then `instances/deploy`, then polls until the deploy completes (~2 to 3 minutes).
...
Change anything (instance name, sync config, replication connection) and re-run `terraform plan` then `apply`. Every update goes through PowerSync's deploy endpoint and triggers a full redeploy (~2 to 3 minutes), which is a constraint of the management API: there is no separate "rename" or "patch" endpoint, so any change is treated as a redeploy. The instance ID stays the same; only the configuration is updated in place.

To force a recreate (e.g. moving regions, which is `ForceNew` on the instance), Terraform plans a destroy, then a create.
